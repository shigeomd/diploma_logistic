...
Процедура ДокументыПриЗаписи(Источник, Отказ) Экспорт
	...
	УзелОбмена = ПолучитьУзелОбмена(Перечисления.НазначениеУзлаОбмена.Документы);
...
//  ПланОбменаСсылка.СинхронизацияКафка - Получить узел обмена
Функция ПолучитьУзелОбмена(Назначение) Экспорт
	
	СтруктураОтвет = Новый Структура("Ссылка,АдресСервера,Топик",ПланыОбмена.СинхронизацияКафка.ПустаяСсылка(),"","");
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
...
Функция ВыгрузитьСправочникиНаСервер() Экспорт
	...
	Для Каждого СтрокаПараметр ИЗ ТаблицаПараметров Цикл
		...
		Если НЕ СтруктураРезультат.Успех Тогда
			ЗаписьЖурналаРегистрации("ОбменЛогистика.ВыгрузкаСправочников", УровеньЖурналаРегистрации.Ошибка,,,СтруктураРезультат.ОписаниеОшибки);
			Возврат СтруктураОтправитель;
		КонецЕсли;
	КонецЦикла;
	...
	Если НЕ СтруктураИнициализация.Успех Тогда
		ЗаписьЖурналаРегистрации("ОбменЛогистика.ВыгрузкаСправочников", УровеньЖурналаРегистрации.Ошибка,,,СтруктураИнициализация.ОписаниеОшибки);
		Возврат СтруктураОтправитель;
	КонецЕсли;
	...
	Если СтруктураРезультат.Успех Тогда
		ЗаписьЖурналаРегистрации("ОбменЛогистика.ВыгрузкаСправочников", УровеньЖурналаРегистрации.Информация,,,"Успех");
...
Функция ПолучательПолучитьСообщенияИзПулаJSON(Компонента, Знач булПолучатьBase64) Экспорт
	...
	СтруктураРезультат.стрСообщения = Компонента.ReceiveJSONMessages(булПолучатьBase64);
