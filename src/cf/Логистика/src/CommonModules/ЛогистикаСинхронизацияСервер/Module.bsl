...
Функция ПолучитьУзелОбмена(Назначение) Экспорт
	...
	СтруктураОтвет = Новый Структура("Ссылка,АдресСервера,Топик,Назначение",ПланыОбмена.СинхронизацияКафка.ПустаяСсылка(),"","",Назначение);
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
...
Функция ЗагрузитьОбъектыНаСервер(СтруктураУзелОбмена) Экспорт
	...
	ИдентификаторСклада = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнойСклад, "ИдентификаторСклада");
